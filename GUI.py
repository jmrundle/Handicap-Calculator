...
class MainWindow(Tk):
    ...
    @staticmethod
    def disp_msg(msg):
        messagebox.showinfo("Handicap Calculator", msg)
...
class MainScreen(Frame):
    ...
    def load_widgets(self):
        ...
        # --------------------------------- Table keys ----------------------------
        # Label(canvas_frame, text="Course", font=("Calibri", 15, 'underline')).grid(row=0, column=0, sticky='nsew')
        # Label(canvas_frame, text='Date', font=("Calibri", 15, 'underline')).grid(row=0, column=1, sticky='nsew')
        # Label(canvas_frame, text='CR/SR', font=("Calibri", 15, 'underline')).grid(row=0, column=2, sticky='nsew')
        # Label(canvas_frame, text="Score", font=("Calibri", 15, 'underline')).grid(row=0, column=3, sticky='nsew')
        # Label(canvas_frame, text="Diff", font=("Calibri", 15, 'underline')).grid(row=0, column=4, sticky='nsew')
        ...
        for i, round in enumerate(self.rounds):
            round_id, _, _, course_name, date, _, cr_sr, score, diff, _ = round
            col = cols[i % 2]
            info = [course_name, date, cr_sr, score, diff]

            for j, item in enumerate(info):
                l = Label(self.canvas_frame, text=str(item), bg=col, font=("Calibri", 14), padx=7, wraplength=140)
      ...
      # changing date column
        ...
        if col == 3:
            try:
                ...
                cr, sr = ACCOUNT_MANAGER.get_round_info(round_id)[6].split('/')
                cr = float(cr)
                sr = int(sr)
    ...
    def delete_round(self, id):
        ...
        ans = messagebox.askyesno("Handicap Calculator", "Would you like to delete this round?\n\n{}: {} ({})".format(info[3], info[7], info[4]))
        if ans:
...
class CourseSearch(Frame):
    ...
    def load_widgets(self):
        ...
        self.button = Button(self, text='BACK', font=("Calibri", 25), bg='#8cd9b3', bd=3, command=lambda *args: (self.forget_widgets(),
                                                                                       self.window.set_frame(MainScreen)))
...
class EnterScore(Frame):
    ...
    def load_widgets(self):
        ...
        self.listbox = Listbox(self.left_subframe, yscrollcommand=scrollbar.set, font=('Calibri', 24), justify=CENTER)
        scrollbar.config(command=self.listbox.yview)
        scrollbar.pack(side=RIGHT, fill=Y)
        ...
        label = Label(date_frame, text="Date", font=("Calibri", 20))
        ...
        button = Button(self.right_subframe, text="Upload", command=self.set_score, padx=5, pady=5, font=("Calibri", 20))

        date_frame.pack(side=TOP, pady=20)
        score_frame.pack(side=TOP, pady=20)
        button.pack(side=TOP, pady=70)
